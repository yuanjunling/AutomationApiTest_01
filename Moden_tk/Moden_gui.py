    ...
    if Performance_test_swing_duration > 0:
        root.after(
            ...
            lambda: performance_test(
                Performance_test_swing_duration, Performance_testing_drop_range
            ),
        )
    else:
        value = max(sqf_list) - min(sqf_list)
...
def Reliability_test(seconds_remaining, Performance_testing_drop_range):
    ...
    if seconds_remaining > 0:
        root.after(
            1000,
            lambda: Reliability_test(seconds_remaining, Performance_testing_drop_range),
        )
    else:
        ...
        if sqf_list:
            ...
            if round(proportion, 2) < int(contents[6]):
                ...
                messagebox.showerror(
                    "错误", f"稳定性超出的比例大于设定值{contents[11]}%,稳定性测试失败"
                )
                OutPut_entry.insert(
                    tk.END,
