...
def Reliability_button():
    ...
    run_time_entry.insert(0, run_times)
    time_out()
    Reliability_test(int(contents[10]),int(contents[9]))
...
def static_test():
    ...
    if ave - 0.5 <= value <= ave + 0.5:
        ...
        OutPut_entry.delete(1.0,tk.END)
        OutPut_entry.insert(tk.END, f"{print_time}:静态信噪比测试通过 当前平均值{ave}\n")
        # HoverLineChartApp(root,sqf_list)
        return True
    else:
        OutPut_entry.delete(1.0, tk.END)
        print_time = f"当前时间：{current_time} "
        OutPut_entry.insert(tk.END, f"{print_time}:静态信噪比测试未通过 当前平均值{ave}")
        messagebox.showerror("错误", "静态信噪比测试未通过")
        return False

def performance_preparation_test(Performance_testing_drop_range,Exceeding_Extreme_Values):
    #性能测试准备工作
    value = max(sqf_list)-min(sqf_list)
    values=round(value,2)
    print(Performance_testing_drop_range)
    if values>float(Performance_testing_drop_range):
        OutPut_entry.insert(tk.END, f"性能测试跌落范围超常 性能测试跌落范围值{value:.2f}\n")
        if values>float(Exceeding_Extreme_Values):
            ...
            results=[x for x in rounded_result if float(Performance_testing_drop_range) <float(x)]
        ...
        OutPut_entry.insert(tk.END, f"静态测试跌落范围正常 跌落范围值{value:.2f}\n开始进行性能测试......\n")
        sqf_list.clear()
        performance_test(int(contents[5]),int(contents[9]))

def performance_test(Performance_test_swing_duration,Performance_testing_drop_range):
    #性能测试
    api_value = api_modem_type()
    cn_entry.delete(0, tk.END)
    sqf = f"{float(api_value['sqf']) * 0.1:.2f}"
    ...
    if Performance_test_swing_duration > 0:
        root.after(1000, lambda: performance_test(Performance_test_swing_duration,Performance_testing_drop_range))
    else:
        value = max(sqf_list) - min(sqf_list)
        values = round(value, 2)
        ...
        if sqf_list:
            ...
            if round(proportion,2)<int(contents[6]):
                OutPut_entry.insert(tk.END, f"性能超出的比例小于设定值{contents[6]}%,性能测试通过\n")
            else:
                messagebox.showerror("错误", f"性能超出的比例大于设定值{contents[6]}%,性能测试失败")
                OutPut_entry.insert(tk.END, f"性能超出的比例大于设定值{contents[6]}%,性能测试失败\n")
...
def Reliability_test(seconds_remaining,Performance_testing_drop_range):
    #可靠性的测试
    api_value = api_modem_type()
    cn_entry.delete(0, tk.END)
    sqf = f"{float(api_value['sqf']) * 0.1:.2f}"
    ...
    seconds_remaining -= 1
    if seconds_remaining > 0:
        root.after(1000, lambda: Reliability_test(seconds_remaining,Performance_testing_drop_range))
    else:
        OutPut_entry.insert(tk.END, f"稳定性测试完成......\n")
        max_value = max(sqf_list)
        result = [round(max_value, 2) - round(x, 2) for x in sqf_list]
        rounded_result = [round(num, 2) for num in result]
        print("result:", rounded_result)
        ...
        if sqf_list:
            ...
            if round(proportion,2)<int(contents[6]):
                OutPut_entry.insert(tk.END, f"稳定性超出的比例小于设定值{contents[11]}%,稳定性测试通过\n")
            else:
                messagebox.showerror("错误", f"稳定性超出的比例大于设定值{contents[11]}%,稳定性测试失败")
                OutPut_entry.insert(tk.END, f"稳定性超出的比例大于设定值{contents[11]}%,稳定性测试失败\n")
...
def open_file():
    #打开文件
    filename = filedialog.askopenfilename(title='打开txt文件', filetypes=[('txt files', '*.txt')])
    global contents
    if filename:
...
def api_modem_type():
    ...
    try:
        if contents[1]=="休斯":
            ...
            return {"esn":esn,"satLong":satLong,"outroute_freq":outroute_freq,"symbol_rate":symbol_rate,"sqf":sqf}
        elif contents[1]=="503":
...
def snmp_get(oid:str):
    ...
    cmdgen.UdpTransportTarget((target_host, 161)), oid)
...
standard_cn_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
cn_max_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
cn_max_entry.grid(row=3, column=1)

cn_min_label = tk.Label(root, text="信噪最小值:", width=10)
cn_min_label.grid(row=4, column=0)
cn_min_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
modem_type_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
modem_sn_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
entry_filename = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
jttest_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
jttest_entry.grid(row=6, column=4)

xntest_label = tk.Label(root, text="性能测试，测试时长:", width=20)
xntest_label.grid(row=7, column=3)
xntest_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
kekaotest_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
kekaotest_entry.grid(row=8, column=4)

xntestyq_label = tk.Label(root, text="性能摇摆台要求:", width=20)
xntestyq_label.grid(row=9, column=3)
xntestyq_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
...
kekaotestyq_entry = tk.Entry(root, font=('calibri', 10, 'bold'), background='white', width=20)
kekaotestyq_entry.grid(row=10, column=4)

# #输出结果
OutPut_entry = scrolledtext.ScrolledText(root,width=80, height=15, font=('calibri', 10, 'bold'),undo=True, autoseparators=False)
...
Reliability_button=tk.Button(root, text="稳定性运行", command=Reliability_button)
